...
{   
	...
	$split_dir = split ('[,.-]', $_POST["uid"]); //返回一个Array,你可以用for读出来
for($i=0;$i<count($split_dir);$i++)
{  
	$sql = "insert into bw_zxwdhf(uid,content,title,xxlx,addtime,lang) values(".$split_dir[i].",'".$content."','".$title."',".$xxlx.",'".date("Y-m-d H:i:s")."','".$Lang."')";
		die($sql);
		//mysql_query($sql); 

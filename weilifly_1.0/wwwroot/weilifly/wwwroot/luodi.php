			...
			<td>
				<div id="content">
					<div id="left">
						<h2>落地转化栏目</h2>
						<ul>
							<li><a href="luodi.php?types=微信营销">微信营销</a></li>
							<li><a href="luodi.php?types=企业APP">企业APP</a></li>
							<li><a href="luodi.php?types=移动营销全案服务">移动营销全案服务</a></li>
						</ul>
					</div>
					<div id="right">
						<h2>落地转化案例简介</h2>
					    <ul class="news">
							...
							if (isset($_GET['types'])) {
								$type=$_GET['types'];
								// 获取总数据量
								$sql = "SELECT count(*) AS amount FROM luodi WHERE type='$type'";
								$res = mysql_query($sql);
								...
								if( $page == 1 ){
								   ...
								   $page_string = '<a href=luodi.php?page=1>第一页</a>|<a href=luodi.php?page='.($page-1).'&types='.$type.'>上一页</a>|';
								}
								if( ($page == $page_count) || ($page_count == 0) ){
								   $page_string2= '下一页|尾页';
								}else{
								   $page_string2 = '<a href=luodi.php?page='.($page+1).'&types='.$type.'>下一页</a>|<a href=luodi.php?page='.$page_count.'&types='.$type.'>尾页</a>';
								}
								$sqlnews="SELECT * FROM luodi where type='$type' LIMIT ". ($page-1)*$pagesize .", $pagesize";

							}else{
								// 获取总数据量
								$sql = "SELECT count(*) AS amount FROM luodi WHERE type='微信营销'";
								$res = mysql_query($sql);
								While($rows = mysql_fetch_array($res)){
									$amount=$rows['amount']; 
								}
								...
								if( $page == 1 ){
								   ...
								   $page_string = '<a href=luodi.php?page=1>第一页</a>|<a href=luodi.php?page='.($page-1).'&types=趋势>上一页</a>|';
								}
								if( ($page == $page_count) || ($page_count == 0) ){
								   $page_string2= '下一页|尾页';
								}else{
								   $page_string2 = '<a href=luodi.php?page='.($page+1).'&types=趋势>下一页</a>|<a href=luodi.php?page='.$page_count.'&types=趋势>尾页</a>';
								}
								$sqlnews="SELECT * FROM luodi where type='微信营销' LIMIT ". ($page-1)*$pagesize .", $pagesize";
							}
							...
							While($row = mysql_fetch_array($result))
							  ...
							  echo "<a href='luodi_list.php?id=".$row['id']."' target='_blank'><h3>".$row['luodi_name']."</h3></a>";
							  echo mb_substr($row['luodi_introduce'], 0, 41,'utf-8');
							  echo "......";
							  echo "</div>";
							  echo "</li>";
							  }

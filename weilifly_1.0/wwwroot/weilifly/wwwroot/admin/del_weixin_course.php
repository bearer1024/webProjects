...
if(isset($_GET['id'])){
	$id=$_GET['id'];
	$sql="DELETE FROM weixin_course WHERE id='$id'";
	mysql_query($sql);
	echo "删除了一条记录！";
}
?>
...
<div>
	<table>
	<tr>
		<td>新闻ID</td><td>课程内容前20字</td><td>删除</td>
	</tr>
		<?php
			$pagesize=6;//每页显示信息
			// 获取当前页数
			...
			if( $page == 1 ){
			   ...
			   $page_string = '<a href=del_weixin_course.php?page=1>第一页</a>|<a href=del_weixin_course.php?page='.($page-1).'>上一页</a>|';
			}
			if( ($page == $page_count) || ($page_count == 0) ){
			   $page_string2= '下一页|尾页';
			}else{
			   $page_string2 = '<a href=del_weixin_course.php?page='.($page+1).'>下一页</a>|<a href=del_weixin_course.php?page='.$page_count.'>尾页</a>';
			}
			...
			While($row = mysql_fetch_array($result))
			  {
			    ...
			    echo "<td>".$row['id']."</td>";
			    echo "<td>";
			    echo mb_substr($row['course_content'], 0, 23,'utf-8');
			    echo "</td>";
			    echo "<td><a href='del_weixin_course.php?id=".$row['id']."'>删除</a></td>";
			    echo "</tr>";
			  }
			mysql_close($con);
			?>

			...
			<td>
				<div id="content">
					<div id="left">
						<h2>动态分类</h2>
						<ul>
							<li><a href="news.php?types=趋势">趋势研究最新报告</a></li>
							<li><a href="news.php?types=微力">深圳微力科技动态</a></li>
							<li><a href="news.php?types=江西移动">江西移动营销动态</a></li>
						</ul>
					</div>
					<div id="right">
						<h2>最新消息 The latest news</h2>
					    <ul class="news">
							<?php
							...
							if (isset($_GET['types'])) {
								$type=$_GET['types'];
								// 获取总数据量
								$sql = "SELECT count(*) AS amount FROM news WHERE news_type='$type'";
								$res = mysql_query($sql);
								...
								if( $page == 1 ){
								   ...
								   $page_string = '<a href=news.php?page=1>第一页</a>|<a href=news.php?page='.($page-1).'&types='.$type.'>上一页</a>|';
								}
								if( ($page == $page_count) || ($page_count == 0) ){
								   $page_string2= '下一页|尾页';
								}else{
								   $page_string2 = '<a href=news.php?page='.($page+1).'&types='.$type.'>下一页</a>|<a href=news.php?page='.$page_count.'&types='.$type.'>尾页</a>';
								}
								$sqlnews="SELECT * FROM news where news_type='$type' ORDER BY time DESC LIMIT ". ($page-1)*$pagesize .", $pagesize";

							}else{
								// 获取总数据量
								$sql = "SELECT count(*) AS amount FROM news WHERE news_type='趋势'";
								$res = mysql_query($sql);
								While($rows = mysql_fetch_array($res)){
									$amount=$rows['amount']; 
								}
								...
								if( $page == 1 ){
								   ...
								   $page_string = '<a href=news.php?page=1>第一页</a>|<a href=news.php?page='.($page-1).'&types=趋势>上一页</a>|';
								}
								if( ($page == $page_count) || ($page_count == 0) ){
								   $page_string2= '下一页|尾页';
								}else{
								   $page_string2 = '<a href=news.php?page='.($page+1).'&types=趋势>下一页</a>|<a href=news.php?page='.$page_count.'&types=趋势>尾页</a>';
								}
								$sqlnews="SELECT * FROM news where news_type='趋势' ORDER BY time DESC LIMIT ". ($page-1)*$pagesize .", $pagesize";
							}
							...
							While($row = mysql_fetch_array($result))
							  ...
							  echo "<a href='news_list.php?id=".$row['news_id']."' target='_blank'><img src=./getdata.php?id=".$row['news_id']."></a>";	
							  echo "</div>";
							  echo "<div class='newscontent'>";
							  echo "<div id='showtime'>".$row['time']."</div>";
							  echo "<a href='news_list.php?id=".$row['news_id']."' target='_blank'><h3>".$row['title']."</h3></a>";	
							  echo mb_substr($row['news_content'], 0, 41,'utf-8');
							  echo "......";
							  echo "</div>";
							  echo "</li>";
							  }

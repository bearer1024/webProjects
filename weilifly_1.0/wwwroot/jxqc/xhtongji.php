		...
		<div id="vip_top">统计信息列表</div>
			<?php
				include_once("connect.php");
					...
					// 获取总数据量
					$sql = "SELECT count(*) AS amount FROM xh_tongji";
					...
					if( $page == 1 ){
					   ...
					   $page_string = '<a href=xhtongji.php?page=1>第一页</a>|<a href=xhtongji.php?page='.($page-1).'>上一页</a>|';
					}
					if( ($page == $page_count) || ($page_count == 0) ){
					   $page_string2= '下一页|尾页';
					}else{
					   $page_string2 = '<a href=xhtongji.php?page='.($page+1).'>下一页</a>|<a href=xhtongji.php?page='.$page_count.'>尾页</a>';
					}
					$sqlnews="SELECT * FROM xh_tongji ORDER BY time DESC LIMIT ". ($page-1)*$pagesize .", $pagesize";

					$result = mysqli_query($con,$sqlnews);
					While($row = mysqli_fetch_array($result))
					  {
					  echo "<div class='every_vip'>";	
					  echo "<a href='tongji_list.php?id=".$row['id']."' target='_blank'><h3>标题：".$row['title']."</h3></a>";
